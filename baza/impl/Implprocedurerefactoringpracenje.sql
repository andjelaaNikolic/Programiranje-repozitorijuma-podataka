...
**              Pracenje treninga
**  DATOTEKA:   ImplProcedureRefactoringPracenje.sql
**  OPIS:       Refaktorisane stored procedure za entitet PRACENJE.
**              Primenjeni principi u odnosu na prethodnu verziju:
**                (а) SRP: svaka procedura radi jednu stvar
**                (б) THROW umesto RAISERROR
**                (в) @ProcName dinamicki naziv procedure
**                (г) SET XACT_ABORT ON
**              Kreirane procedure:
**                (1) impl.PromeniPracenja → orkestracija
**  AUTOR:      Andjela (DbDeveloper)
**  NAPOMENA:   Pokrenuti nakon ImplProcedurePracenje.sql
*/
...
-- =====================================================
-- 1. impl.PromeniPracenja  [REFAKTORISANA VERZIJA]
--    Procedura za sinhronizaciju pracenja klijenta.
--    Uloga: iskljucivo orkestracija:
--      (a) odredjuje @ProcName za log
--      (b) izvrsava MERGE u transakciji
--      (c) u slucaju greske loguje i prosledjuje THROW
-- =====================================================
...
BEGIN
    ...
    BEGIN TRY
        BEGIN TRANSACTION;
            ...
            -- (b) Sinhronizacija pracenja pomocu MERGE
...
PRINT '------------------------------------------------------------------';
PRINT N' impl.PromeniPracenja (refactoring verzija)';
